...
const BNPLList = ({ timeBracket }: { timeBracket: String }) => {
  ...
  const token = localStorage.getItem("token");
  const [fetchSingleShipment, { data: shipmentData, isLoading: dataLaoding }] =
    useFetchSingleShipmentMutation();
            ...
            {tableData.length > 0 ? (
              tableData
                ...
                .map((item: any, index: number) => (
                  <BNPLCard key={index} item={item} clickedCard={clickedCard} />
                ))
            ) : (
              <></>
            )}
          </Box>
          <Flex justify={"center"} flex={0.2} p={".5rem"}>
            <CreditScoreCard />
          </Flex>
        </Flex>
      )}
