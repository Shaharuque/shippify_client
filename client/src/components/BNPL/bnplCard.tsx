...
const BNPLCard: React.FC<TrackingItemProps> = ({
  item,
  clickedCard,
}: {
  item: IUpcomingPayments;
  clickedCard: any;
}) => {
  console.log(item);
  return (
    ...
    >
      <Flex gap={"2rem"}>
        ...
        <Stack align={"center"} w={"10vw"} whiteSpace={"nowrap"}>
          <Text fontWeight="bold">Payment Deadline</Text>
          <Text fontSize="sm">
            {moment(new Date(item?.payments.paymentDeadline))?.format(
              "MM-DD-YYYY"
            )}
          </Text>
        </Stack>
