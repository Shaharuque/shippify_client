...
const ShipmentCard = ({ shipment, clickedCard }: IShipment) => {
  ...
  return (
    ...
    >
      <Flex gap={"1vw"} align={"center"} whiteSpace={"nowrap"}>
        <Stack align={"center"} w={"120px"} textAlign={"center"}>
          <Image
            src={
              shipment?.rateDetail?.carrier_code === "ups"
                ? ups
                : shipment?.rateDetail?.carrier_code === "fedex"
                ? fedex
                : stamps
            }
            boxSize={"2.5rem"}
          ...
          <Text fontWeight={"md"} fontSize={"sm"} whiteSpace={"pre-wrap"}>
            {shipment?.rateDetail?.service_type}
          </Text>
        </Stack>
        <Stack align={"center"} w={"80px"}>
        ...
        <Stack align={"center"} w={"100px"}>
          ...
          <Text fontSize={"sm"}>
            {shipment?.shipment_detail?.packages.length}
          </Text>
        </Stack>
        <Stack align={"center"} w={"120px"}>
        ...
        <Stack align={"center"} w={"100px"}>
          ...
          <Text fontSize={"sm"}>
            {shipment?.shipment_detail?.ship_from?.city_locality},{" "}
            {shipment?.shipment_detail?.ship_from?.country_code}
          </Text>
        </Stack>
        <Stack align={"center"} w={"100px"}>
          <Text fontWeight={"bold"}>Destination</Text>
          <Text fontSize={"sm"}>
            {shipment?.shipment_detail?.ship_to?.city_locality},{" "}
            {shipment?.shipment_detail?.ship_to?.country_code}
          </Text>
        </Stack>
        <Stack align={"center"} w={"120px"}>
          <Text fontWeight={"bold"}>Delivery Date</Text>
          <Text fontSize={"sm"}>
            {moment(shipment?.rateDetail?.estimated_delivery_date)?.format(
              "MM-DD-YYYY"
            )}
          </Text>
        </Stack>
        <Stack align={"center"} w={"120px"}>
          ...
          <Badge colorScheme="pink" borderRadius={"md"} fontSize={"xs"}>
