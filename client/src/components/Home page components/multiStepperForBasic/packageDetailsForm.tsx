...
const PackageDetailsForm = ({
  ...
  const packages = useAppSelector(
    (state: RootState) => state.shipments.packages
  );
  const sender = useAppSelector(
    (state: RootState) => state.shipments.ship_from
  );
  const reciever = useAppSelector(
    (state: RootState) => state.shipments.ship_to
  );

  const [defaultPackageValues, setDefaultPackageValues] =
    useState(defaultValues);

  const { control, register, handleSubmit, reset, setValue, getValues } =
    useForm<TPackageDetailsForm>({
  ...
  return (
    <Box p={"2vw"} w={"40rem"}>
      {sender?.country_code !== reciever?.country_code ? (
        <CustomsInfoForm />
      ) : null}
      <PackageNumbers
        packages={packages}
      ...
      <Text as="b" fontSize={"1.25rem"} letterSpacing={0.2}>
        Package Details
      </Text>
      ...
      <form onSubmit={handleSubmit(onSubmit)} style={{ marginTop: "1.5rem" }}>
        {isCustom ? (
          <>
            ...
            <Flex gap={"2rem"} m={"1rem 0"} alignItems={"center"}>
              <FormControl isRequired>
                <Controller
                  ...
                  render={({ field }) => (
                    <>
                      <NumberInput precision={2} max={70}>
                        <NumberInputField
          ...
          <Flex gap={"2rem"} alignItems={"center"} m={"1rem 0"}>
            <FormControl isRequired>
              <Select
                border={"1px solid #314866"}
                transition={"all 0.30s ease-in-out;"}
        ...
        <Flex gap={"2rem"} alignItems={"center"} m={"1rem 0"}>
          <FormControl isRequired>
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <NumberInput precision={2} max={150}>
                  <NumberInputField
                    {...field}
                    placeholder="Weight"
                    border={"1px solid #314866"}
        ...
        <Flex justify={"flex-end"} m={"2rem 0"} gap={"1rem"}>
          <BackButton onClick={() => prevStep()} width="8rem" />
          <RegularButton
            text="Continue"
            width="12rem"
